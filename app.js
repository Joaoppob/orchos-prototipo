...
function render() {
  ...
  if (sync) { sync.textContent = state.agentOffline ? 'Reconectando…' : 'Sincronizado · nuvem'; sync.classList.toggle('is-offline', state.agentOffline); }
...
const APPS = {
  ...
  ajustes: {
    ...
    body() {
      ...
      const sw = (id, label, style) => `<button class="wallswatch${w === id ? ' is-active' : ''}" data-wall="${id}" style="${style}" aria-label="${label}"><span>${label}</span></button>`;
      const seg = (opts, active) => `<div class="seg">${opts.map((o) => `<button class="${o === active ? 'is-active' : ''}">${o}</button>`).join('')}</div>`;
      return `
      <div class="settings__section"><div class="settings__head">Aparência</div>
        <div class="settings">
          <div class="setting setting--col"><span class="setting__label">Wallpaper<small class="setting__sub">fundo do desktop · tons de cinza</small></span>
            <div class="wallpicker">${sw('bisao', 'Bisão', "background-image:url('assets/bg.jpg')")}${sw('slit', 'Slit-scan', "background-image:url('assets/background.jpg')")}${sw('dots', 'Pontos', "background-image:url('assets/background1.jpg')")}${sw('solido', 'Sólido', 'background:linear-gradient(180deg,#2c2c33,#1d1d22)')}</div>
            <div class="wallmeta mono" id="wallMeta">${WALL_META[w]}</div>
          </div>
          <div class="setting"><span class="setting__label">Tema<small class="setting__sub">macOS neutro · sem cor de acento</small></span>${seg(['Claro', 'Escuro', 'Sistema'], 'Escuro')}</div>
          <div class="setting"><span class="setting__label">Desfocar wallpaper<small class="setting__sub">vidro sobre o desktop</small></span><button class="switch" data-on="false" data-key="blur" aria-label="Desfocar wallpaper"></button></div>
        </div>
      </div>
      ...
      <div class="settings__section"><div class="settings__head">Conta</div>
        <div class="settings__about">
          ...
          <div class="row"><span>Assistente ativo</span><span>Durin · orquestrador</span></div>
          <div class="row"><span>Plano</span><span>OrchOS Max</span></div>
          <div class="row"><span>Sincronização</span><span>Sincronizado · nuvem</span></div>
        </div>
      </div>
      ...
      <div class="settings__section"><div class="settings__head">Sobre</div>
        <div class="settings__about">
          <div class="row"><span>Versão</span><span>OrchOS 2031.1</span></div>
    ...
    wire(root) {
      ...
      root.querySelectorAll('.switch').forEach((sw) =>
        sw.addEventListener('click', () => {
          ...
          if (sw.dataset.key === 'blur') document.querySelector('.wallpaper').classList.toggle('is-blurred', !on);
        }));
      root.querySelectorAll('.wallswatch').forEach((s) =>
        s.addEventListener('click', () => {
          root.querySelectorAll('.wallswatch').forEach((x) => x.classList.remove('is-active'));
          s.classList.add('is-active');
          const meta = root.querySelector('#wallMeta');
          if (meta) meta.textContent = WALL_META[s.dataset.wall];
          setState({ wallpaper: s.dataset.wall });
        }));
...
function wire() {
  ...
  // Esc → volta um nível na janela focada
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape') popView(); });
